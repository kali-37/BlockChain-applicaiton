...
export const authenticateWithWallet = async (walletAddress: string): Promise<AuthTokens> => {
  try {
    // Get nonce from server
    const nonceResponse = await api.get<NonceResponse>(`/api/auth/nonce/${walletAddress}/`);
    ...
    const authResponse = await api.post<AuthTokens>('/api/auth/authenticate/', {
...
export const refreshAccessToken = async (): Promise<boolean> => {
  ...
  try {
    const response = await api.post<{access_token: string, expires_at: string}>('/api/auth/refresh/', {
      refresh_token
    });
