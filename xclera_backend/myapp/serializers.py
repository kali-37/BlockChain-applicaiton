...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserProfile
        fields = [
            'id', 'username', 'country', 'phone_number', 'email',
            'wallet_address', 'referrer', 'referrer_username',
            'current_level', 'direct_referrals_count', 'max_referral_depth',
            'is_registered_on_chain', 'date_registered', 'is_profile_complete'
        ]
        read_only_fields = ['id', 'direct_referrals_count', 'max_referral_depth', 
                           'is_registered_on_chain', 'date_registered', 'wallet_address']
...
class ProfileUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if self.partial:
            ...
            if any(field not in data for field in ['username', 'country', 'phone_number']):
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Transaction
        fields = [
            'id', 'user', 'user_username', 'transaction_type', 'amount',
            'level', 'recipient', 'recipient_username', 'transaction_hash',
...
class ReferralRelationshipSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'user', 'user_username', 'upline', 'upline_username', 'level', 'date_created']
...
class RegistrationSerializer(serializers.Serializer):
    ...
    private_key = serializers.CharField(write_only=True, style={'input_type': 'password'})
    
    # Optional profile fields
    username = serializers.CharField(max_length=100, required=False, allow_null=True, allow_blank=True)
