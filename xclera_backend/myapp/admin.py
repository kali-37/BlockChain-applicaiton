...
class UserProfileAdmin(ReadOnlyModelAdmin):
    list_display = (
        "display_name",
        "wallet_address",
        "get_referrer",
    ...
    def get_referrer(self, obj):
        if obj.referrer:
            display_name = (
                obj.referrer.username or f"{obj.referrer.wallet_address[:10]}..."
            )
    ...
    def get_readonly_fields(self, request, obj=None):
        # If this is an existing object, prevent changing critical fields , obj= None for new object creation so all fields are editable in that case though it contradic
        # t the above has_add_permission method but it is defensive programming :)
        if obj:
...
class TransactionAdmin(ReadOnlyModelAdmin):
    ...
    def get_recipient(self, obj):
        if obj.recipient:
            display_name = (
                obj.recipient.username or f"{obj.recipient.wallet_address[:10]}..."
            )
